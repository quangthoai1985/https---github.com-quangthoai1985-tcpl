...
async function deleteCollection(collectionPath: string, batchSize: number = 50) {
    console.log(`Step 2: Deleting all documents from collection '${collectionPath}'...`);
...
async function main() {
  try {
    ...
    console.log("NOTE: Seeding for most collections has been disabled as per user request to avoid overwriting data.");
    
    // The user has requested to NOT seed data anymore to avoid overwriting their work.
    // The following lines are commented out to prevent accidental data loss.
    // If you need to re-seed the database, uncomment the relevant lines.
    ...
    //     if (assessmentData.approverId === undefined) {
