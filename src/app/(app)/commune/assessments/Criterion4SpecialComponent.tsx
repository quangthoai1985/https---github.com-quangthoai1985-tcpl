...
const Criterion4SpecialComponent = ({ 
    ...
    const firstIndicatorId = indicator.id;
    if (!firstIndicatorId || !assessmentData[firstIndicatorId]) return null;

    const data = assessmentData[firstIndicatorId];
                                ...
                                <CardContent className="space-y-4">
                                     {assignmentType === 'quantity' && (!indicator.assignedDocumentsCount || indicator.assignedDocumentsCount === 0) && (
                                        <div className="grid gap-2 p-3 border rounded-md bg-background">
                                            <Label htmlFor="communeDocCount">Tổng số Kế hoạch đã ban hành</Label>
                                                ...
                                                <div key={index} className="p-3 border-l-4 border-blue-300 rounded-r-md bg-background text-sm">
                                                     ...
                                                     {assignmentType === 'specific' ? (
                                                        ...
                                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                                            <div className="grid gap-1.5"><Label htmlFor={`doc-name-${index}`}>Tên Kế hoạch</Label><Input id={`doc-name-${index}`} value={doc.name} onChange={(e) => handleLocalDocDetailChange(index, 'name', e.target.value)} /></div>
                                                ...
                                                return (
                                                    <div key={docIndex} className="p-3 border rounded-lg grid gap-2 bg-background">
                                                        ...
                                                        <CT4EvidenceUploader 
                                                            ...
                                                            contentId={content.id} // Pass contentId here
                                                            evidence={evidence} 
