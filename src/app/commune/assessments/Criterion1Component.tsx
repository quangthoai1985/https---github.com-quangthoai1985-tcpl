...
const Criterion1Component = ({ 
    ...
    // BẮT ĐẦU CODE LOG MỚI
    console.log("Criterion1Component received criterion:", JSON.stringify(criterion, null, 2));
    console.log("Criterion1Component received assessmentData keys:", Object.keys(assessmentData || {}));
    const firstIndicatorId = criterion.indicators?.[0]?.id;
    console.log("Criterion1Component firstIndicatorId:", firstIndicatorId);
    console.log("Criterion1Component assessmentData for first indicator:", assessmentData?.[firstIndicatorId!]);
    // KẾT THÚC CODE LOG MỚI

    // Dòng code gốc kiểm tra điều kiện
    if (!firstIndicatorId || !assessmentData || !assessmentData[firstIndicatorId]) {
         console.log("Criterion1Component returning null due to missing data."); // Thêm log ở đây nữa
         return null;
    }
